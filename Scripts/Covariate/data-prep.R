...
# Setup for Covariate Model
# Need smoothed versions of ebird counts and effort
# Need coordinates for ebird data
coords=coordinates(block[,c('Lon2','Lat2')])
#fit gam to counts and effort, the s() just ensures it's a smooth function of lat and long
EE = mgcv::gam(block$avg.eHours~s(coords[,1],coords[,2],k=9))$fitted.values
YY = mgcv::gam(block$avg.eCount~s(coords[,1],coords[,2],k=9))$fitted.values

# Visualize, if needed
# require(raster)
# plot(raster::rasterFromXYZ(cbind(coords, YY)))
# plot(raster::rasterFromXYZ(cbind(coords, EE)))
### ### ### ### ### ### ### ### ###
### ### ### ### ### ### ### ### ###
